...
class VerbatimAnalyzer:
    ...
    def _prepare(self, df) -> pl.LazyFrame:
        ...
        Seule étape qui peut nécessiter un collect() : le parsing de listes string.
        """
        ...
        # Si c'est un LazyFrame, on doit vérifier le type de la colonne verbatim
        # Pour ça on collecte UN SEUL row
        if isinstance(df, pl.LazyFrame):
            sample_row = df.select(self.verbatim_col).head(1).collect()
            col_dtype = sample_row[self.verbatim_col].dtype
            is_string_col = col_dtype == pl.Utf8 or col_dtype == pl.String
            is_list_col = col_dtype.base_type() == pl.List

            if is_string_col:
                # Les verbatims sont des strings repr de listes
                # On DOIT collecter pour parser avec Python, pas le choix
                print("   Collecte pour parsing des listes string...")
                df = df.collect()
            elif is_list_col:
                # Déjà des listes → tout peut rester lazy
                lf = df.with_columns(
                    pl.col(self.verbatim_col).list.join(". ").alias("verbatim"),
                    pl.col(self.verbatim_col).list.len().alias("n_messages"),
                ).with_columns(
                    _clean_expr("verbatim").alias("clean")
                ).filter(
                    pl.col("clean").str.len_chars() > 2
                )
                # Cast date si nécessaire
                lf = lf.with_columns(
                    pl.col(self.date_col).cast(pl.Date)
                )
                print("   ✅ Préparation 100% lazy (colonnes déjà en liste)")
                return lf
            else:
                df = df.collect()

        # ── À ce stade, df est un DataFrame (collecté) ──
        # Cas string : parsing Python nécessaire
        if df[self.verbatim_col].dtype == pl.Utf8 or df[self.verbatim_col].dtype == pl.String:
        ...
        # Repasser en lazy pour la suite
        print("   ✅ Données préparées → conversion en LazyFrame")
        return df.lazy()
