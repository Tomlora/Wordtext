...
class VerbatimAnalyzer:
    ...
    def _prepare(self, df: pd.DataFrame) -> pd.DataFrame:
        """Explose les listes de verbatims en une ligne par verbatim."""
        ...
        # Explosion : une ligne par verbatim
        df = df.explode(self.verbatim_col).dropna(subset=[self.verbatim_col])
        df = df.rename(columns={self.verbatim_col: "verbatim"})
        df["verbatim"] = df["verbatim"].astype(str).str.strip()
        df = df[df["verbatim"].str.len() > 2]  # filtre les verbatims trop courts
    ...
    def detect_negative_spikes(self) -> pd.DataFrame:
        ...
        Retourne un DataFrame journalier avec :
            - total_verbatims, negative_count, negative_ratio
            - z_score, is_spike
        """
        ...
        daily = (
            ...
            .agg(
                total_verbatims=("verbatim", "count"),
                negative_count=("is_negative", "sum"),
                mean_neg_score=("neg_score", "mean"),
            )
            .reset_index()
        )
        daily.columns = ["date", "total_verbatims", "negative_count", "mean_neg_score"]
        daily["date"] = pd.to_datetime(daily["date"])
        daily["negative_ratio"] = daily["negative_count"] / daily["total_verbatims"]
    ...
    def run_full_analysis(self, save_dashboard: Optional[str] = None) -> dict:
        ...
        print(f"📝 Nombre de verbatims : {len(self.df):,}")
        print()
...
if __name__ == "__main__":
    ...
    normal_verbatims = [
        ...
        "Je veux des informations sur mon contrat",
        "Pouvez-vous me rappeler",
        "J'aimerais changer mon mot de passe",
        "Comment accéder à mon espace client",
        "Je voudrais un conseiller",
    ]
    negative_verbatims = [
        "C'est scandaleux personne ne me répond",
        "Ça fait trois semaines que j'attends",
        "Service incompétent je vais résilier",
        "Panne depuis ce matin rien ne fonctionne",
        "Bug sur l'application impossible de se connecter",
        "Je suis très déçu de votre service",
        "Arnaque pure et simple remboursez-moi",
        "Votre service est lamentable",
        "Je vais aller chez le concurrent",
        "Surfacturation sur ma dernière facture",
    ]
    # Termes injectés pour tester la détection d'émergence
    emerging_verbatims = [
        "Problème avec la nouvelle mise à jour fibre",
        "Depuis la migration fibre plus rien ne marche",
        "Migration fibre catastrophique",
    ]
    ...
    for d in dates:
        ...
        day_verbatims = list(
            np.random.choice(normal_verbatims, size=int(n * 0.75))
        ) + list(np.random.choice(negative_verbatims, size=int(n * 0.25)))

        # Simuler un pic le 15 janvier 2025
        if d.date() == pd.Timestamp("2025-01-15").date():
            day_verbatims += list(
                np.random.choice(negative_verbatims, size=100)
            )

        # Simuler des termes émergents les 2 dernières semaines
        if d >= pd.Timestamp("2025-02-12"):
            day_verbatims += list(
                np.random.choice(emerging_verbatims, size=30)
            )

        rows.append({"date": d, "verbatims": day_verbatims})
